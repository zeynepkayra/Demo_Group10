...
Our e-commerce database has the following entities: Customers, Products, Categories, Suppliers, Order Details, Transactions, and Ads. The E-R diagram can be seen in Figure 1.

![ER Diagram](final_er_diagram.jpg)  

\newpage

* For the Customer entity, each customer has a unique customer ID as its primary key, customer name, unique mobile number, home address, credit card number, and unique email address. The Name attribute is a composite of the two attributes named First Name and Last Name. The Address attribute is also a composite of the four attributes: Postcode, House No, Street Name, and Country. We assume that customers can only save one address and one credit card number simultaneously.
...
* For the Categories entity, each category has a unique category ID as its primary key and category name.

* For the Order Details entity, each order detail has a unique order detail ID as its primary key, discount, and delivery date. The order Details entity has one foreign key, Transaction ID, to ensure order details can be correctly traced back to their related transactions.

* For the Suppliers entity, each supplier has a unique supplier ID as its primary key, supplier name, address, and rating. The Address attribute is also a composite of the four attributes: Postcode, House No, Street Name, and Country.

* For the Transactions entity, each transaction has a unique transaction ID as its primary key and payment method. The transactions entity has one foreign key Order Detail ID, which means each transaction will correspond to one or more order details.

* For the Ads entity, each ad has a unique ad ID as its primary key, duration, and cost. The ad entity has one foreign key, Product ID, showing that each ad promotes a specific product.
...
2.  Each category can have multiple products, and each product can be under multiple categories, such as blender under kitchen and electrical appliances (M to N).

3.  Each product can have multiple ads, but an ad can be for a sole product (1 to N).

4.  Each customer can order multiple products, and each order can be bought by different customers (M to N).

5.  Each customer can have multiple order details, but each order detail can only be under one customer (1 to N).

6.  Each product can be within multiple order details, and each order detail can have many products (M to N).

7.  Each transaction can have multiple order details, but each order detail only belongs to one transaction (1 to N).

The relationship set can be seen in Figure 2.

![Relationship Sets](relationshipSets.jpg)
...
The logical schema given below outlines the fundamental components of our e-commerce database: customer, product, category, supplier, ads, order, order detail and transaction, as well as their attributes and relationship tables. This is the textual description of our database tables and their columns:
...
Using the logical schema, the physical schema is generated using RSQLite in R. It can be seen in Appendix A.
...
For example, a standard address consists of many parts, including postcode, house number, street name, and country. When generating address data, we separate these parts into four columns to maintain a single value in each column. Similarly, we separated names into last names and first names. Also, inside the database, total price of an order is also kept. This is a derived column calculated as sum of product_price * quantity for all products purchased inside an order. Since it distorts normalization, total price column is stored inside a temporary view and updated generically once new orders are placed by customers. At the end of this stage, our synthetic data is normalized and adheres to the rules of 1NF, 2NF and 3NF. 
...
![Payment method comparision](payment.jpeg){width=400 fig-align="center"} 
...
![Products by category](best_sellers.jpg){width=400 fig-align="center"}
...
![Popular category ](top_category.jpeg){width=400 fig-align="center"}
...
![Distribution of orders](basket_price.jpeg){width=400 fig-align="center"}
...
## Appendix B. Synthetic Data Generation and Import

First phase data load
